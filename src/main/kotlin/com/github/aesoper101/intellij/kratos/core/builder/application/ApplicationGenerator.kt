...
class ApplicationGenerator(private val appDirectory: PsiDirectory, private val appName: String) {
    fun doGenerate() {
        val module = ModuleUtil.findModuleForPsiElement(appDirectory)
        val project = appDirectory.project
        val projectDirectory = LocalFileSystem.getInstance().findFileByIoFile(File(project.presentableUrl!!))
        if (module != null && projectDirectory != null) {
